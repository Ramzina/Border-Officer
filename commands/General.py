...
####################################################################################################################################
	...
	async def kys(
		...
		if member.mention == interaction.user.mention:
			embed = discord.Embed(title="! Error !",timestamp=datetime.utcnow(), color=Color.from_rgb(27, 152, 250))
			embed.add_field(name="Command:", value=f"`{interaction.command}`")
			embed.add_field(
				name="Error:", value="`Passed member is the same as the message author`"
			)
			await interaction.followup.send(
				embed=embed.set_thumbnail(url=interaction.user.avatar_url)
			)
			return
		embed = discord.Embed(
			title="! **• KYS ALERT •** !",
			description=f"\n- Alert {member.mention}, {interaction.user.mention} thinks you should impale yourself on a railing!\n:O\n",
			timestamp=datetime.utcnow(),
			color=Color.from_rgb(27, 152, 250),
		)

		embed.set_footer(text=f"Border hopping hispanic")
...
####################################################################################################################################
	...
	async def random(
		...
		embed = discord.Embed(
			title="**Random**",
			description=f"> Number: {random.randint(num1, num2)}",
			color=Color.green(),
		)
		await interaction.followup.send(
			embed=embed, ephemeral=True
...
####################################################################################################################################
	...
	async def eightball(self,interaction:discord.Interaction, question:str):
		...
		possible_answers = ['Yes.', 'No.', 'Possibly.', 'Maybe.', 'The stars tell me yes.', 'The stars tell me no.', 'The stars are unsure.', 'The stars don\'t know yet.']

		answer = random.choice(possible_answers)

		embed = discord.Embed(title='**The magic 8ball**', description=f'- {interaction.user.mention} asked: "{question}"\n- Magic 8ball says: "{answer}"', color=Color.from_rgb(27, 152, 250))

		await interaction.followup.send(embed=embed.set_thumbnail(url=interaction.user.avatar.url))
