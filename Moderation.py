...
####################################################################################################################################
    ...
    async def ban(
        self, interaction: discord.Interaction, member: discord.User, reason: str = None, ephemeral:bool=False
   ):
    ...
    @app_commands.command(description="Unbans the passed user.")
    @app_commands.default_permissions(ban_members=True, manage_roles=True)
    async def unban(self, interaction: discord.Interaction, member: discord.User, ephemeral:bool=False):
        print("[Untrueban] has just been executed")
        await interaction.response.defer(thinking=True)
        ...
        try:
            ...
            await interaction.followup.send(embed=discord.Embed(
                title='**Unban**',
                description=f"> Unbanned: {member.mention}",color=Color.green()), ephemeral=ephemeral)
            embed2 = discord.Embed(
                title="! **• UNBAN •** !",
                description=f"- 🧍 Issuer: {interaction.user.mention}\n- 🧍 User: {member.mention}\n- 🪪 User name: {member}\n- 🪪 User Id: `{member.id}`",
                timestamp=datetime.utcnow(),
        ...
        except commands.UnknownBan:
            await interaction.followup.send(
                f"{interaction.user}, {member} is not banned.", ephemeral=True
            )
...
####################################################################################################################################
    ...
    @app_commands.default_permissions(mute_members=True)
    async def unmute(self, interaction: discord.Interaction, member: discord.Member, ephemeral:bool=False):
...
####################################################################################################################################
    ...
    async def kick(
        self, interaction: discord.Interaction, member: discord.Member, reason: str=None, ephemeral:bool=False
    ) -> None:
...
####################################################################################################################################
    ...
    async def clear(
        self, interaction: discord.Interaction, member: discord.User, amount: int, ephemeral:bool=False
    ):
        ...
        await interaction.followup.send(embed=discord.Embed(title='**Clear**', description=f'> Messages cleared: `{amount}`',color=Color.green()), ephemeral=ephemeral)
...
####################################################################################################################################
    ...
    async def purge(self, interaction: discord.Interaction, amount: int = 5, ephemeral:bool=False) -> None:
...
####################################################################################################################################
    ...
    @app_commands.default_permissions(mute_members=True)
    async def blacklist(self, interaction: discord.Interaction, member: discord.Member, reason: str, ephemeral:bool=False) -> None:
    ...
    @app_commands.default_permissions(mute_members=True)
    async def unblacklist(self, interaction: discord.Interaction, member: discord.User, ephemeral:bool=False):
        print("[Unblacklist] has just been executed")

        role = discord.utils.find(
            lambda r: r.name == "| BLACKLISTED", interaction.guild.roles
        )
        await interaction.response.defer(thinking=True)
        db = await aiosqlite.connect("config.db")
...
####################################################################################################################################
    ...
    async def addrole(
        self, interaction: discord.Interaction, member: discord.User, role: discord.Role, ephemeral:bool=False
    ) -> None:
